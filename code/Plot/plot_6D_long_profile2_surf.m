function plot_6D_long_profile2_surf(phasespace,qm,Ef,nbin)
% Plot long profile + size & divergences
...
smooth=filter(ones(1,windowSize)/windowSize,1,sx);sx=smooth;
smooth=filter(ones(1,windowSize)/windowSize,1,sz);sz=smooth;
smooth=filter(ones(1,windowSize)/windowSize,1,divx);divx=smooth;
smooth=filter(ones(1,windowSize)/windowSize,1,divz);divz=smooth;
...
set(0,'DefaultFigurePosition',[1 scrsz(4)/2 scrsz(3)/1.5 scrsz(4)/1.2]);
...
subplot(3,2,5)
set(gca,'FontSize',14)
plot(gs*1e3 ,sx*1e6,'-r'); hold on ;plot(gs*1e3 ,sz*1e6,'-b'); hold off
xlabel('S (mm) ');ylabel('rms size (µm)');
legend('X','Z')
xlim(limx);
grid on;
subplot(3,2,6)
set(gca,'FontSize',14)
plot(gs*1e3 ,divx*1e3,'-r'); hold on ;plot(gs*1e3 ,divz*1e3,'-b'); hold off
xlabel('S (mm) ');ylabel('rms div (mrad)');
legend('X','Z')
xlim(limx);
grid on;
