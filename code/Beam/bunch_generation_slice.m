function [phasespace,qm]=bunch_generation_slice(twiss,disp,eps,long,np,q,sigm)
% Give 1,2,3,4,6 dim phasespace with gaussian electron distribution
%      5 is constant distribution, the slice
...
% 
phasespace(5,:)= 2-rand(1,np); 

% remove particles out of sigm
if nargin==7
    ind = abs(phasespace(6,:))<sigm;
    phasespace=phasespace(:,ind) ;
    ind = abs(phasespace(5,:))<sigm;
    phasespace=phasespace(:,ind) ;
    ind = abs(phasespace(4,:))<sigm;
    phasespace=phasespace(:,ind) ;
    ind = abs(phasespace(3,:))<sigm;
    phasespace=phasespace(:,ind) ;
    ind = abs(phasespace(2,:))<sigm;
    phasespace=phasespace(:,ind) ;
    ind = abs(phasespace(1,:))<sigm;
    phasespace=phasespace(:,ind) ;
end
